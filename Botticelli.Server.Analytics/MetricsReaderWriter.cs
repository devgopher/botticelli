...
public class MetricsReaderWriter
{
    ...
    public MetricsReaderWriter(MetricsContext context)
    {
        _context = context;
    }
    ...
    {
        return await _context.MetricModels
                             .AsAsyncEnumerable()
                             .Where(func)
                             .CountAsync(token);
    }
    ...
    {
        return _context.MetricModels.Select(x => x.Name).Distinct();
    }
    ...
    {
        var frames = DateTimeUtils.GetRange(from,
                                            to,
                                            frameLength,
                                            token);

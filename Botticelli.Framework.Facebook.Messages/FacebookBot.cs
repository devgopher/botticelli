...
using Botticelli.Framework.Facebook.Messages.API.Requests;
...
namespace Botticelli.Framework.Facebook.Messages;

public class FacebookBot : BaseBot<FacebookBot>
{
    ...
    public FacebookBot(
        ...
        ILogger<FacebookBot> logger) : base(logger, metrics)
    {
        _secureStorage = secureStorage;
    }

    public override BotType Type => BotType.Facebook;
    ...
    private async Task<IEnumerable<FacebookMessageRequest>> CreateRequestsWithAttachments(SendMessageRequest request,
        string peerId,
        CancellationToken token)
    {
        var currentContext = _secureStorage.GetBotContext(BotDataUtils.GetBotId());
        var result = new List<FacebookMessageRequest>(100);
        var first = true;

        if (request.Message.Attachments == default)
        {
            var vkRequest = new FacebookMessageRequest
            {
